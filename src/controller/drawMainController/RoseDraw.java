...
public class RoseDraw {
	...
	public RoseDraw(MousePoint p1, MousePoint p2, Color color){
		rose=new Rose();
		rose.setSize(200);
		rose.setNum(1);
		...
		for(int j = 0; j < 2000 ; j++)	//// 按任意键退出
		{
			for(int i = 0; i < 10000; i++)	 // 
				if(calc(Math.random(), Math.random(),(random.nextInt(10000) )% 46 / 0.74, dot))
				{
					...
					//putpixel(x - 25, y - 20, RGB(r, b, g));
					g2.setColor(new Color(r, g, b));
					for(int i1=0;i1<rose.getNum();i1++) {
						g2.drawLine(x-25+i1*200, y-25, x-25+i1*200, y-25);
